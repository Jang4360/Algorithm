	...
	public static void main(String[] args) throws IOException {
		...
		for (int l = 0; l<layer; l++) {
			...
			for (int i = l; i<N-l-1; i++) {
				layers.add(arr[i][M-l-1]);
			}
			
			// bottom
			for (int j = M-l-1; j>l; j--) {
				layers.add(arr[N-l-1][j]);
			}
			...
			int rotate = R%size;
			List<Integer> rotateLayers = new ArrayList<>();
			for (int i = 0; i<layers.size(); i++) {
				rotateLayers.add(layers.get((i+rotate)%size));
			}
			
			int index = 0;
			// top
			for (int j = l; j<M-l-1; j++) arr[l][j] = rotateLayers.get(index++);

			// right
			for (int i = l; i<N-l-1; i++) arr[i][M-l-1] = rotateLayers.get(index++);

			// bottom
			for (int j = M-l-1; j>l; j--) arr[N-l-1][j] = rotateLayers.get(index++);
			
			// left
			for (int i = N-l-1; i>l; i--) arr[i][l] = rotateLayers.get(index++);
